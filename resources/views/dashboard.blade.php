    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="flex justify-between items-center">
                ...
                <a href="{{ route('cars.create') }}" class="px-4 py-2 text-white bg-green-600  rounded hover:bg-green-400">Add Car</a>
            </div>
            @if($cars->isEmpty())
                        ...
                        <div class="bg-white shadow rounded-lg overflow-hidden flex flex-col">
                            ...
                            <div class="p-4 flex-1 flex flex-col justify-between">
                                ...
                                <div class="mt-4 flex space-x-2">
                                    <a href="{{ route('cars.edit', $car) }}" class="px-2 py-1 bg-green-600 text-white text-sm rounded hover:bg-green-400">Edit</a>
                                    <form action="{{ route('cars.destroy', $car) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="px-2 py-1 bg-red-600 text-white text-sm rounded hover:bg-red-400">Delete</button>
                                    </form>
                                    <form action="{{ route('cars.toggleRent', $car) }}" method="POST" class="inline">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit"
                                                class="px-2 py-1 bg-yellow-400  text-white text-sm rounded hover:bg-yellow-500">
                                ...
                                <div class="ml-4 flex flex-col space-y-2">
                                    ...
                                    @else
                                        <form action="{{ route('rentals.accept', $request) }}" method="POST">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="px-4 py-2 bg-green-600 text-sm rounded hover:bg-green-700">
                                                Accept
                                            </button>
                                        </form>
